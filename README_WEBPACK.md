...
#### [Entrypoints.config.js](entrypoints.config.js)

The configuration for entrypoints for this project is contained within [entrypoints.config.js](entrypoints.config.js)

  <br>

This file is responsible for getting the names of the files that are in the `pages` directory and passing those into the expected entrypoints format that webpack expects.

That's where we've also defined what module dependencies are shared between bundles.
E.g. - React, ReactDOM

More in-depth information can be found in webpack's documentation:
